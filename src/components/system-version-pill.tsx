...
export function SystemVersionPill({ isOwner = false }: { isOwner?: boolean }) {
  ...
  const modalContent = modalOpen && mounted ? (
    <div
      className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 bg-black/60 backdrop-blur-xs animate-in fade-in duration-150 overflow-y-auto"
      onClick={(e) => {
    ...
    >
      <div className="relative w-full max-w-xl my-auto bg-white dark:bg-zinc-900 rounded-2xl border border-zinc-200 dark:border-zinc-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
        {/* Modal Header */}
        <div className="px-6 py-4 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
          ...
          <button
            ...
            disabled={updating}
            className="text-zinc-400 hover:text-zinc-600 dark:hover:text-zinc-200 p-1 rounded-lg hover:bg-zinc-100 dark:hover:bg-zinc-800 cursor-pointer disabled:opacity-40 transition-colors"
        ...
        <div className="p-6 overflow-y-auto space-y-6 flex-1 text-xs">
          ...
          {isUpdateAvailable ? (
            <div className="p-4 rounded-xl bg-amber-50 dark:bg-amber-950/40 border border-amber-300 dark:border-amber-800 flex items-start gap-3 text-amber-900 dark:text-amber-200">
              <IconRocket className="w-5 h-5 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
              <div>
            ...
            <div className="p-4 rounded-xl bg-zinc-100 dark:bg-zinc-800/80 border border-zinc-300 dark:border-zinc-700 flex items-start gap-3 text-zinc-800 dark:text-zinc-200">
              <IconWifiOff className="w-5 h-5 text-zinc-500 shrink-0 mt-0.5" />
              <div>
            ...
            <div className="p-4 rounded-xl bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-300 dark:border-emerald-800 flex items-start gap-3 text-emerald-900 dark:text-emerald-200">
              <IconCheckCircle className="w-5 h-5 text-emerald-600 dark:text-emerald-400 shrink-0 mt-0.5" />
              <div>
  ...
  return (
    <>
      ...
      {/* Render modal directly into document.body to ensure perfect viewport centering */}
